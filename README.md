...
p.s. Добавил ci уже после дедлайна.
...
Линтер запускается как отдельный инструмен:
...
Линтер можно запускать с флагом `-fix` для автоматического исправления нарушений в логах:
...
linters:
  ...
  enable:
    - customlinter
```
(в настоящий момент настроенная интеграция через plugin golangci-lint не реализована)
